...
export class TagService {
  ...
  private getTagsUrl = 'https://api.stackexchange.com/2.2/tags?order=desc&sort=popular&site=stackoverflow';
  ...
  getTags(page: Number, filter: String) : Observable<Tag[]> {
    ...
    let options = new RequestOptions({ headers: headers });
    return this.http.get(this.getTagsUrl).map(this.extractData)
      .catch(this.handleError);
  }
